...
export const stripEmptyKeys = (obj) =>
  ...
  const buildGroup = (group, acc) => {
    ...
    const parsed = isParameterException ? buildException(content) : buildRegular({ relation, scope });
  ...
  const includeCrawler = (stash, layer) => {
    ...
    const groups = [...missing, ...layer].map((group) => buildGroup(group, stash));
...
export const buildWhereKey = (groups) => {
 ...
 export const buildIncludeKey = groups => distributeBackwards(groups).reduce(includeCrawler, {}).include
