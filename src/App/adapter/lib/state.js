...
export function mergeState(inits, diffs) {
  const useful = diffs.reduce((acc, scope) => [...acc, scope.label, scope.group], [LABEL_FOR_CONFIG ])
  const usefulInits = inits.filter(
    ({label}) =>
      useful.includes(label)
  )
